...
for i=2:size(t_laser,1)
    ...
    for n=1:nparticles
        ...
        for beam=1:2         
            ...
            % get ray to grid intersection
            ang1 = atan2(ogymax-y_particle(n), ogxmax-x_particle(n));
            ang2 = atan2(ogymax-y_particle(n), ogxmin-x_particle(n));
            ang3 = atan2(ogymin-y_particle(n), ogxmin-x_particle(n)) + 2*pi;
            ang4 = atan2(ogymin-y_particle(n), ogxmax-x_particle(n)) + 2*pi;
            
            % ray/laser angle 
            ang = theta_particle(n) + angles(1,j); 
            ang = mod(ang, 2*pi);
            if ang > pi
                ang = ang - 2*pi;
            elseif ang < -pi
            ...
            if angg > ang1 && angg <= ang2 % top
                ptx = x_particle(n) + (ogymax - y_particle(n)) / tan(ang);
                pty = ogymax;
            elseif angg > ang2 && angg <= ang3 % left
                ptx = ogxmin;
                pty = y_particle(n) + (ogxmin - x_particle(n)) * tan(ang);
            elseif angg > ang3 && angg <= ang4% bottom
                ptx = x_particle(n) + (ogymin - y_particle(n)) / tan(ang);
                pty = ogymin;
            else  %right 
                ptx = ogxmax;
                pty = y_particle(n) + (ogxmax - x_particle(n)) * tan(ang);
            end
                
            % find candidate hit points
            dx = ogres * cos(ang);
            dy = ogres * sin(ang);
            if abs(dx) >= abs(dy)
                nn = (ptx - x_particle(n)) / dx;
            else
                nn = (pty - y_particle(n)) / dy;
            end
            nn = floor(nn);
            
            pre_x = linspace(x_particle(n), ptx, nn);
            pre_y = linspace(y_particle(n), pty, nn);

            masks = true(size(pre_x));
            masks(pre_x < ogxmin) = false;
            masks(pre_x > ogxmax) = false;
            masks(pre_y < ogymin) = false;
            masks(pre_y > ogymax) = false;

            x_idx = (pre_x(masks) - ogxmin) / ogres;
            y_idx = (pre_y(masks) - ogymin) / ogres;
            x_idx = round(x_idx);
            x_idx = min(max(x_idx,1),ognx);
            y_idx = round(y_idx);
            y_idx = min(max(y_idx,1),ogny);

            idx = sub2ind(size(ogp), y_idx, x_idx);
            cand_vals = ogp(idx);
            cand_mask = cand_vals > 0.3;    % linear index mask
            
            % find the first hit 
            x_coords = pre_x(cand_mask);
            y_coords = pre_y(cand_mask);
            dist = sqrt((x_coords-x_particle(n)).^2 + (y_coords-y_particle(n)).^2);
            yp_laser = min(dist);
            if yp_laser > y_laser_max
                continue;
            end
            ...
            % evaluate correction       
            prob = 1/sqrt(2*pi*laser_var) * exp(-(yp_laser-y_laser(i,j))^2/laser_var/2.0);
            w_particle(n) = w_particle(n) * prob * w_gain;
