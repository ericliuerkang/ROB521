...
for i=1:5:size(t_laser,1)
    ...
    for pt = 1:npoints % Loop through all laser points at a single timestamp
        % Get robot orientation at current timestamp and assign its
        % associated rotation matrix to put laser scan in robot frame to
        % map frame
        curScan = y_laser(i, pt);
        robotAng = theta_interp(i);
        relativeAng = angles(pt);
        R = [cos(robotAng), -sin(robotAng); sin(robotAng), cos(robotAng)];
        
        % For ranges that smaller than laser scan measurement, compensate
        % its belief function so it's less likely to be occupied.
        for range = r_min_laser:ogres:r_max_laser
            if range < curScan
                % Transform the empty location from robot frame to map
                % frame using rotation matrix computed above
                rangePt = [x_interp(i); y_interp(i)] + R * (range * [cos(relativeAng); ...
                    sin(relativeAng)] - [0.1; 0]) - [ogxmin; ogymin];
                rangePixel = round(rangePt / ogres);
                
                % Penalize for empty region
                oglo(rangePixel(2), rangePixel(1)) = ...
                    oglo(rangePixel(2), rangePixel(1)) - 1;
            end
        end
        
        % If laser scan measurement not valid, skip
        if isnan(curScan)
            continue;
        end
        
        % For actual valid laser scan measuerement, update its belief
        % function so it's more likely to be occupied
        % Transform the valid scan from robot frame to map
        % frame using rotation matrix computed above
        scanPt = [x_interp(i); y_interp(i)] + R * (curScan * [cos(relativeAng); ...
            sin(relativeAng)] - [0.1; 0]) - [ogxmin; ogymin];
        scanPixel = round(scanPt / ogres);

        % Penalize for empty region
        oglo(scanPixel(2), scanPixel(1)) = ...
            oglo(scanPixel(2), scanPixel(1)) + 5;
    end
    
    % After processing all valid laser scans, try to turn belief function
    % into actual probabilities
