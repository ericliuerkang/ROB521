...
for i=2:numodom
    ...
    x_odom(i) = x_odom(i-1) + h*v_odom(i)*cos(theta_odom(i-1));
    y_odom(i) = y_odom(i-1) + h*v_odom(i)*sin(theta_odom(i-1));
...
%         disp(y_laser(i,:))
        if abs(omega_interp(i)) < 0.1
            x_tmp = (((cos(angles+theta_interp(i))).*y_laser(i,:))') +  x_interp(i) - 0.1*cos(theta_interp(i));
            y_tmp = (((sin(angles+theta_interp(i))).*y_laser(i,:))') +  y_interp(i) - 0.1*sin(theta_interp(i));
